...
export default function AddExpense() {
  ...
  const currentUser: string = profile?.emailId ?? "";
  const groupId = params.groupId;
  const [loading, setLoading] = useState(false);
  const [alert, setAlert] = useState(false);
  const [alertMessage, setAlertMessage] = useState("");
  var [groupMembers, setGroupMembers] = useState<string[]>([]);
  var [groupCurrency, setGroupCurrency] = useState<CurrencyType>(CurrencyType.USD);
  ...
  const formik = useFormik({
    ...
    onSubmit: async () => {
      ...
      if (await addExpenseService(values, setAlert, setAlertMessage))
        window.location.href = configData.VIEW_GROUP_URL + groupId;
    },
  });
